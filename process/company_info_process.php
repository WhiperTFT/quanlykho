...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $upload_dir = __DIR__ . '/../uploads/company/'; // Thư mục lưu trữ file upload (ví dụ: quanlykho/uploads/company/)
    if (!is_dir($upload_dir)) {
        mkdir($upload_dir, 0775, true);
    }

    // Lấy thông tin công ty hiện tại để biết đường dẫn file cũ (nếu có)
    ...
    if (isset($_FILES['logo']) && $_FILES['logo']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($logo_extension, $allowed_logo_extensions)) {
            // Xóa logo cũ nếu có và không phải là file mặc định
            ...
            if (move_uploaded_file($logo_file['tmp_name'], $logo_destination)) {
                $logo_path_to_save = 'uploads/company/' . $new_logo_filename; // Lưu đường dẫn tương đối từ gốc web
            } else {
        ...
        // Xóa logo nếu người dùng chọn xóa và có logo hiện tại
        if ($current_logo_path && file_exists(__DIR__ . '/../' . $current_logo_path)) {
            unlink(__DIR__ . '/../' . $current_logo_path);
        }
    ...
    if (isset($_FILES['signature']) && $_FILES['signature']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($signature_extension === 'png') {
            ...
            if (move_uploaded_file($signature_file['tmp_name'], $signature_destination)) {
                $signature_path_to_save = 'uploads/company/' . $new_signature_filename; // Lưu đường dẫn tương đối từ gốc web
            } else {
        ...
        // Xóa chữ ký nếu người dùng chọn xóa và có chữ ký hiện tại
        if ($current_signature_path && file_exists(__DIR__ . '/../' . $current_signature_path)) {
            unlink(__DIR__ . '/../' . $current_signature_path);
        }
    ...
    try {
        ...
        // Ghi log lỗi chi tiết thay vì hiển thị cho người dùng
    ...
    // Nếu không phải POST request, chuyển hướng về trang chính hoặc trang thông tin
    header('Location: ../company_info.php');
    exit;
}
?>