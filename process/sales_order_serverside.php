<?php
// File: process/sales_order_serverside.php (Đã sửa lỗi HY093 và loại bỏ ký tự lạ)
...
try {
    ...
    $column_map = [
        1 => 'so.order_number',
        2 => 'so.order_date', // Sử dụng tên cột DB cho sắp xếp và lọc server-side
        3 => 'p.name',
    ...
    // Sắp xếp
    $order_column_index = $_POST['order'][0]['column'] ?? 1; // Mặc định cột số đơn hàng (index 1)
    $order_dir = $_POST['order'][0]['dir'] ?? 'desc';
    // Ánh xạ index cột sang tên cột DB, mặc định là 'so.order_number' nếu index không hợp lệ
    ...
    $filter_order_number = $_POST['columns'][1]['search']['value'] ?? '';
    $filter_order_date = $_POST['columns'][2]['search']['value'] ?? '';
    $filter_supplier_name = $_POST['columns'][3]['search']['value'] ?? '';
    ...
    // Phần FROM và JOIN cơ bản (luôn JOIN partners)
    $sql_from_join = "
        FROM sales_orders so
        JOIN partners p ON so.supplier_id = p.id AND p.type = 'supplier'
    ";

    // Nếu cần JOIN bảng chi tiết cho bộ lọc item details, thêm vào đây (LEFT JOIN để không bỏ sót đơn hàng không có item)
    if ($use_details_join) {
        $sql_from_join .= " LEFT JOIN sales_order_details sod ON so.id = sod.order_id";
    }
    ...
    $columnFilters = []; // Mảng tạm chứa các điều kiện dạng chuỗi SQL (ví dụ: "so.order_number LIKE :filter_number")
    ...
    if (!empty($searchValue)) {
        ...
        // $where .= " WHERE (so.order_number LIKE :global_search OR p.name LIKE :global_search)"; // Ví dụ
    ...
    if (!empty($filter_order_number)) {
        $columnFilters[] = "so.order_number LIKE :filter_order_number";
        $columnParams[':filter_order_number'] = "%" . $filter_order_number . "%";
    }
    // Xử lý định dạng ngày "dd/mm/yyyy" từ bộ lọc ngày cụ thể
    if (!empty($filter_order_date)) {
        $dateObj = DateTime::createFromFormat('d/m/Y', $filter_order_date);
        if ($dateObj) { // Kiểm tra xem định dạng ngày có đúng không
            $columnFilters[] = "DATE(so.order_date) = :filter_exact_date";
            $columnParams[':filter_exact_date'] = $dateObj->format('Y-m-d'); // Chuyển vềYYYY-MM-DD
        }
    }

    // Thêm các bộ lọc cột khác ở đây (supplier_name, status, etc.)
    if (!empty($filter_supplier_name)) {
        $columnFilters[] = "p.name LIKE :filter_supplier";
        $columnParams[':filter_supplier'] = "%" . $filter_supplier_name . "%";
    }
    ...
    if (!empty($filterYear)) {
        $columnFilters[] = "YEAR(so.order_date) = :filter_year";
    ...
    if (!empty($filterMonth)) {
        ...
        if ($filterMonthInt >= 1 && $filterMonthInt <= 12) { // Kiểm tra tháng hợp lệ
            $columnFilters[] = "MONTH(so.order_date) = :filter_month";
    ...
    $sql_total = "SELECT COUNT(so.id) FROM sales_orders so JOIN partners p ON so.supplier_id = p.id AND p.type = 'supplier'";
    ...
    $sql_filtered_from_join = " FROM sales_orders so JOIN partners p ON so.supplier_id = p.id AND p.type = 'supplier'"; // Luôn JOIN partners

    if ($use_details_join) {
        $sql_filtered_from_join .= " LEFT JOIN sales_order_details sod ON so.id = sod.order_id"; // JOIN sod nếu lọc item
    }
    ...
    foreach ($data as $row) {
        // Format ngày tháng
        try {
            $row['order_date_formatted'] = !empty($row['order_date']) ? (new DateTime($row['order_date']))->format('d/m/Y') : '-';
        } catch (Exception $e) { $row['order_date_formatted'] = $row['order_date']; } // Giữ nguyên giá trị gốc nếu format lỗi
        // Format tiền tệ
