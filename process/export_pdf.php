...
$pdf_configs = [
    'order' => [
        ...
        'partner_role_on_pdf_key' => 'seller',        // lang key: "Bên Bán" (Nhà cung cấp)
        'own_company_role_on_pdf_key' => 'buyer',     // lang key: "Bên Mua" (Công ty của bạn)
    ...
    'quote' => [ // Báo giá Bán hàng (cho Khách hàng)
        ...
        'partner_role_on_pdf_key' => 'customer',        // lang key: "Khách hàng"
        'own_company_role_on_pdf_key' => 'provider',     // lang key: "Bên Cung Cấp" (Công ty của bạn)
...
$txt_own_company_role_signature = $lang[$current_config['own_company_role_on_pdf_key']] ?? 'Your Company';
$txt_partner_role_signature = $lang[$current_config['partner_role_on_pdf_key']] ?? 'Partner';
        ...
        <table style="width: 100%; border-collapse: collapse; margin-top: 10px; page-break-inside: avoid;">
            <tr>
                <td style="width: 55%; vertical-align: top; padding-right: 15px; border: none !important;">
                    ...
                    $notes_to_display = '';
                    if ($raw_notes_content !== null && is_string($raw_notes_content)) {
                        $current_notes = preg_replace_callback('/\\\\x([0-9a-fA-F]{2})/', function($matches) { return chr(hexdec($matches[1])); }, $raw_notes_content);
                        if ($current_notes === null) { $current_notes = $raw_notes_content; }
                        $plain_text_notes = strip_tags($current_notes);
                        $decoded_notes = html_entity_decode($plain_text_notes, ENT_QUOTES | ENT_HTML5, 'UTF-8');
                        $trimmed_notes = trim($decoded_notes);
                        if ($trimmed_notes !== '') { $notes_to_display = nl2br(htmlspecialchars($trimmed_notes, ENT_QUOTES | ENT_HTML5, 'UTF-8')); }
                    }
                    if (!empty($notes_to_display)):
                    ?>
                        <div> <p style="margin-bottom: 3px; margin-top:0;"><strong><?= htmlspecialchars($txt_notes ?? 'Notes') ?>:</strong></p>
