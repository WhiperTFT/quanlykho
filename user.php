...
global $available_permissions; // Sử dụng global nếu biến được định nghĩa ở phạm vi toàn cục trong init.php
// HOẶC nếu init.php trả về một mảng config, bạn có thể lấy từ đó.
        ...
        if (!has_permission('users_add')) { // Chỉ admin mới có quyền users_add theo logic hiện tại
            ...
            if (empty($username) || empty($password) || empty($role)) {
                ...
                try {
                    ...
                    log_activity($current_user_id, "Thêm người dùng mới: " . $username, $pdo);
                    $action = 'list';
                    ...
                    log_activity($current_user_id, "Lỗi khi thêm người dùng: " . $e->getMessage(), $pdo);
                }
            }
        }
                ...
                if (empty($username)) {
                    ...
                    try {
                        $stmt->execute($params);
                         if ($stmt->rowCount() > 0) {
                            $message = $lang['user_edit_success'] ?? "Cập nhật người dùng thành công!";
                             log_activity($current_user_id, "Cập nhật thông tin người dùng ID: " . $edited_user_id . " (" . $username . ")", $pdo);
                        ...
                        $action = 'list';
                         ...
                         log_activity($current_user_id, "Lỗi khi cập nhật người dùng ID " . $edited_user_id . ": " . $e->getMessage(), $pdo);
                    }
                }
             }
         }
             ...
             if ($current_user_id && $deleted_user_id === $current_user_id) {
                 ...
                 try {
                     $stmt->execute([':id' => $deleted_user_id]);
                      if ($stmt->rowCount() > 0) {
                          $message = $lang['user_delete_success'] ?? "Xóa người dùng thành công!";
                           log_activity($current_user_id, "Xóa người dùng ID: " . $deleted_user_id, $pdo);
                      } else {
                     ...
                     $message = $lang['error_db_delete'] ?? "Lỗi khi xóa người dùng: " . $e->getMessage();
                      ...
                      log_activity($current_user_id, "Lỗi khi xóa người dùng ID " . $deleted_user_id . ": " . $e->getMessage(), $pdo);
                 }
             }
         }
    }
