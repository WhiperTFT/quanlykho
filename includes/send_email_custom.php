...
// Đã sửa đổi để chỉ ghi log vào queue và kích hoạt script nền cho cả order và quote
// Đã BỎ LOG CHI TIẾT cho hàm exec()

// error_log("Endpoint send_email_custom.php was hit at " . date('Y-m-d H:i:s'), 3, 'C:/Windows/Temp/endpoint_hit.log'); // Bạn có thể giữ hoặc bỏ dòng log này tùy ý

// Luôn trả về JSON cho AJAX
...
// Đường dẫn file log cho kết quả exec()
// --- ĐÃ XÓA ĐỊNH NGHĨA $exec_log_file_path VÀ HÀM direct_exec_log ---
...
if (!empty($pdf_url)) {
    ...
    $pdf_path_physical = __DIR__ . "/../" . ltrim($pdf_url, '/'); // Điều chỉnh nếu cấu trúc thư mục khác
