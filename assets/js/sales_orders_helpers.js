...
// Dùng để parse các số khi LOAD từ DB về form (edit/xem).
...
function formatChildRowDetails(details, currency) {
    if (!details || details.length === 0) {
        return `<div class="p-3 text-muted text-center">${LANG['no_items_in_order'] || 'Không tìm thấy sản phẩm.'}</div>`;
    }
    ...
    details.forEach((item, index) => {
        // Chỉ HIỂN THỊ: dữ liệu từ server => parseServerNumber
...
function calculateLineTotal(row) {
    ...
    // Ở input: người dùng gõ -> parseInputNumber
...
// === Hàm Tính Tổng Cộng Hóa Đơn ===
function calculateSummaryTotals() {
    const currentCurrency = currencySelect.val();

    // Đọc VAT từ input rồi ép integer
    ...
    itemTableBody.find('tr').each(function () {
        ...
        if (quantity > 0 && unitPrice >= 0) subTotal += quantity * unitPrice;
    });

    // Làm tròn hiển thị theo tiền tệ
    const roundForCurrency = (val) => (currentCurrency === 'VND'
        ? Math.round(val)            // VND: 0 số lẻ
        : Math.round(val * 100) / 100 // USD (khác): 2 số lẻ
    );
    ...
    // Giá trị submit về backend
    $('#input-subtotal').val(subTotal.toFixed(2));                     // để chính xác
    $('#input-vattotal').val((subTotal * vatRate / 100).toFixed(2));
    $('#input-grandtotal').val((subTotal * (1 + vatRate / 100)).toFixed(2));
    $('#input-vatrate').val(vatRate);                                  // LƯU Ý: integer
}
