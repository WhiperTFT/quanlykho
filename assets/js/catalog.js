...
$(document).ready(function() {
    ...
    if (!categoryModal || !productModal || !viewProductModal) {
        ...
        // Hoặc return để dừng thực thi script nếu modal là cốt lõi
        // return;
    }
    ...
    // Hàm hiển thị thông báo (có thể thay bằng thư viện toast đẹp hơn)
    function showUserMessage(message, type = 'success') {
        // Tạm thời dùng alert, bạn có thể tích hợp thư viện như Toastr, SweetAlert
        alert(type.toUpperCase() + ": " + message);
        // Chỉ reload nếu là thông báo thành công và không phải lỗi validation
        if (type === 'success') {
            // Cân nhắc: Không reload ngay mà cập nhật cây DOM bằng JS sẽ mượt hơn
            location.reload();
        }
    }
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.data) {
                    ...
                    checkFileLimits(); // Kiểm tra và vô hiệu hóa input nếu cần
    ...
    // --- Xử lý File Upload Input Change ---

     // Hàm kiểm tra và cập nhật trạng thái input file
     function checkFileLimits() {
        const currentImages = parseInt($('#productImages').data('current-files') || 0);
        const currentDocs = parseInt($('#productDocuments').data('current-files') || 0);
        const selectedImages = $('#productImages')[0].files ? $('#productImages')[0].files.length : 0;
        const selectedDocs = $('#productDocuments')[0].files ? $('#productDocuments')[0].files.length : 0;

        const totalImages = currentImages + selectedImages;
        const totalDocs = currentDocs + selectedDocs;

        const imgInput = $('#productImages');
        const docInput = $('#productDocuments');
        const imgLabel = $('#currentImagesLabel');
        const docLabel = $('#currentDocumentsLabel');

        if (totalImages >= MAX_IMAGES) {
            imgInput.prop('disabled', true);
            imgLabel.text((LANG['max_files_reached'] || 'Maximum files reached (%s)').replace('%s', MAX_IMAGES));
        } else {
            imgInput.prop('disabled', false);
            imgLabel.text(LANG['current_images'] || 'Current Images:');
        }

         if (totalDocs >= MAX_DOCS) {
            docInput.prop('disabled', true);
             docLabel.text((LANG['max_files_reached'] || 'Maximum files reached (%s)').replace('%s', MAX_DOCS));
        } else {
            docInput.prop('disabled', false);
             docLabel.text(LANG['current_documents'] || 'Current Documents:');
        }
    }
    ...
    $('#productImages').on('change', function(e) {
        ...
        if (files.length + currentFileCount > MAX_IMAGES) {
             errorContainer.text((LANG['can_upload_max_files'] || 'You can upload a maximum of %s files.').replace('%s', MAX_IMAGES));
        ...
        for (let i = 0; i < files.length; i++) {
         ...
         checkFileLimits(); // Cập nhật lại trạng thái input sau khi chọn
    });
     ...
     $('#productDocuments').on('change', function(e) {
        ...
        errorContainer.text('');
         ...
         if (files.length + currentFileCount > MAX_DOCS) {
            errorContainer.text((LANG['can_upload_max_files'] || 'You can upload a maximum of %s files.').replace('%s', MAX_DOCS));
             fileInput.val('');
             checkFileLimits();
            return;
         }
         ...
         checkFileLimits(); // Cập nhật lại trạng thái input
    });
     ...
     function createFileElement(file, isExisting = false) {
         ...
         // Đường dẫn file_path trả về từ server là tương đối từ gốc web
         const filePath = file.file_path;
         let element = '';

         if (!filePath) return ''; // Bỏ qua nếu không có đường dẫn
         ...
         if (file.file_type === 'image') {
             element = `
                 <div class="existing-file-container position-relative d-inline-block border p-1 me-2 mb-2 text-center" data-file-id="${fileId}">
                     <a href="${filePath}" target="_blank" title="${fileName}">
                         <img src="${filePath}" alt="${fileName}" class="img-thumbnail" style="max-width: 80px; max-height: 80px;" onerror="this.src='https://placehold.co/80x80/eee/ccc?text=Error'; this.onerror=null;">
                     </a>
                     <small class="d-block text-muted text-truncate" style="max-width: 80px;">${fileName}</small>
                     ${isExisting ? `
             ...
             element = `
                 <div class="existing-file-container d-flex align-items-center justify-content-between border-bottom pb-1 mb-1" data-file-id="${fileId}">
                     <span class="d-flex align-items-center text-truncate" style="max-width: 85%;">
                         <i class="bi bi-file-earmark-pdf me-2"></i>
                         <a href="${filePath}" target="_blank" title="${LANG['view'] || 'View'} ${fileName}" class="text-truncate">${fileName}</a>
                     </span>
                     ${isExisting ? `
             ...
             $.ajax({
                 ...
                 success: function(response) {
                     if (response.success) {
                         ...
                         if (currentCount > 0) {
                             $(fileInputId).data('current-files', currentCount - 1);
                         }
                         checkFileLimits(); // Kích hoạt lại input nếu cần
                     } else {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success && response.data) {
                    ...
                    if (images.length > 0) {
                        detailsHtml += '<div class="d-flex flex-wrap gap-3 mb-3">'; // Tăng gap
                        images.forEach(img => {
                             detailsHtml += `
                                <div class="text-center border rounded p-1">
                                    <a href="${img.file_path}" data-bs-toggle="tooltip" title="${escapeHtml(img.original_filename)}" target="_blank">
                                        <img src="${img.file_path}" alt="${escapeHtml(img.original_filename)}" class="img-fluid" style="max-width: 150px; max-height: 150px; object-fit: contain;" onerror="this.src='https://placehold.co/150x150/eee/ccc?text=Error'; this.onerror=null;">
                                    </a>
    ...
    // --- (Optional) Kích hoạt Select2 nếu thư viện được nhúng ---
    // if ($.fn.select2) {
    //     console.log('Initializing Select2');
    //     try {
    //         $('#productCategory').select2({
    //             dropdownParent: $('#productModal'), // Quan trọng khi dùng Select2 trong Modal Bootstrap
    //             theme: "bootstrap-5", // Sử dụng theme Bootstrap 5 nếu có
    //             width: '100%'
    //         });
    //          $('#productUnit').select2({
