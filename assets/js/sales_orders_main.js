...
function loadAndApplySavedFilters() {
  ...
  try {
    ...
    if (savedYear !== null && savedYear !== '') {
      devLog("Đang đặt giá trị cho #filterYear: '" + savedYear + "'");
      $('#filterYear').val(savedYear);
    } else {
      devLog("Không có giá trị Năm được lưu, bỏ qua.");
    }

    if (savedMonth !== null && savedMonth !== '') {
      devLog("Đang đặt giá trị cho #filterMonth: '" + savedMonth + "'");
      $('#filterMonth').val(savedMonth);
    } else {
      devLog("Không có giá trị Tháng được lưu, bỏ qua.");
    }
  } catch (e) {
    console.error('Lỗi nghiêm trọng trong hàm loadAndApplySavedFilters:', e);
  }
  devLog("--- Kết thúc hàm loadAndApplySavedFilters ---");
}
...
function populateAndSelectQuoteInDropdown($selectElement, preselectQuoteId = null, preselectQuoteNumber = null) {
  if (!$selectElement.length) {
    devLog("SO Page: Target select element for quotes not found.");
    return;
  }
    ...
    success: function (response) {
      ...
      if (preselectQuoteId) {
        if (quoteForPreselectionActuallyAdded) {
          ...
          devLog(`SO Page: Quote ID ${preselectQuoteId} (Num: ${preselectQuoteNumber}) for preselection not in filtered list. Adding temp.`);
...
$(document).ready(function () {
  devLog("Document ready for Sales Orders. Starting main initialization...");
  loadAndApplySavedFilters();
  initializePage();
  if (typeof setupEventListeners === 'function') { // Từ sales_orders_events.js
    setupEventListeners();
  } else {
    devLog("setupEventListeners (for SO) function not found.");
  }
  ...
  function initializeCKEditor(elementId, configOptions = {}) {
    ...
    if (!element) {
      devLog(`Element with ID #${elementId} not found for CKEditor.`);
      return;
    }
    ...
    ckEditorReady[elementId] = ClassicEditor
      ...
      .then(editor => {
        ...
        try {
          ...
          devLog('CKEditor metadata attach failed:', e);
        }
        ...
        try {
          ...
          devLog('CKEditor live sync to textarea failed:', e);
        }
  ...
  function initializePage() {
    devLog("Initializing Sales Orders page starting...");
    ...
    if (!itemTableBody.length) {
      devLog("SO Main: initializePage - #item-details-body NOT FOUND in DOM!");
    }
    ...
    if (webSignatureSrc) { // webSignatureSrc từ sales_orders_config.js
      ...
      devLog(`Signature visibility restored from localStorage: ${shouldBeVisible}`);
    } else {
      buyerSignatureImg.hide().removeAttr('src');
      toggleSignatureButton.hide();
      devLog("No signature image, hiding toggle button and image.");
    }
    buyerSignatureImg.on('load', function () {
      devLog("Buyer signature image loaded. Dimensions:", this.naturalWidth, "x", this.naturalHeight);
    }).on('error', function () {
      devLog("Failed to load buyer signature image from src: " + $(this).attr('src'));
      $(this).hide();
      toggleSignatureButton.hide();
    });
    ...
    devLog("SO Main: initializePage - END");
  }
  ...
  if (typeof ClassicEditor !== 'undefined') {
    ...
    devLog("CKEditor 5 library (ClassicEditor) not found.");
  }
  // Khởi tạo jQuery UI Draggable cho ảnh chữ ký
  devLog("SO Main: handlePageLoadActions - START");
  devLog("SO Main: handlePageLoadActions - itemTableBody at entry:", itemTableBody); // <<--- THÊM LOG
  ...
  if (typeof $.ui !== 'undefined' && typeof $.ui.draggable !== 'undefined' && buyerSignatureImg.length) {
    try {
      ...
      devLog("Signature draggable initialized after document ready.");
    } catch (e) {
      console.error("Signature Draggable initialization error:", e);
    }
  } else {
    if (!buyerSignatureImg.length) devLog("Buyer signature image element not found for draggable.");
    else devLog("jQuery UI Draggable not found. Signature will not be draggable.");
  }
  devLog("Full page initialization complete.");
    ...
    if (triggerCreateOrder === 'true') {
      ...
      if (dataString) {
        try {
          ...
          devLog("Data loaded from localStorage for SO creation:", quoteData);
          ...
          if (typeof addItemRow === 'function' && Array.isArray(quoteData.items) && quoteData.items.length > 0) {
            ...
            if (!quoteData.items || quoteData.items.length === 0) devLog("No items data found in localStorage to populate order details.");
          }
        } catch (e) {
      ...
      if (!isNaN(orderIdToView) && typeof loadOrderForEdit === 'function') {
        devLog("Loading specific order for view/edit:", orderIdToView);
        loadOrderForEdit(orderIdToView);
      } else {
        if (typeof loadOrderForEdit !== 'function') console.error("loadOrderForEdit function is not defined.");
        else devLog("Invalid orderId found in localStorage for viewing:", viewSpecificOrderId);
      }
    } else {
      const $linkedQuoteSelect = $('#order_quote_id_form');
      if ($linkedQuoteSelect.length) {
  ...
  if (typeof $.ui !== 'undefined' && typeof $.ui.draggable !== 'undefined' && buyerSignatureImg && buyerSignatureImg.length) {
    try {
      ...
      devLog("Signature draggable for order initialized.");
    } catch (e) { console.error("Signature Draggable (order) initialization error:", e); }
  }
  devLog("Full order page initialization complete.");
});
$('#btnSaveOrder').off('click.saveOrder').on('click.saveOrder', async function (e) {
  ...
  devLog('[DEBUG] Notes to save =', payload.notes); // kiểm tra đúng ngay lần 1
