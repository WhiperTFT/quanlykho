<?php
// Thêm dòng này NGAY LẬP TỨC ở ĐẦU FILE để kiểm tra xem script có được chạy không
// Endpoint log file (using error_log for very early logging)
$log_base_dir = __DIR__ . '/logs/'; // Đường dẫn vật lý đến thư mục logs
$endpoint_log_file = $log_base_dir . 'endpoint_hit.log';
// --- ĐÃ XÓA ĐỊNH NGHĨA $exec_log_file_path Ở ĐÂY ---

// --- Bỏ lệnh log hit endpoint nếu không cần nữa ---
// error_log("Endpoint create_email_log.php was hit at " . date('Y-m-d H:i:s') . " from " . ($_SERVER['REMOTE_ADDR'] ?? 'N/A'), 3, $endpoint_log_file);


// File: create_email_log.php - Đã thêm logic kích hoạt worker và log debug
// CHỈNH SỬA: Cập nhật để hỗ trợ cả order và quote

header('Content-Type: application/json');

// Bao gồm file kết nối database của bạn
// !!! CẦN KIỂM TRA VÀ CHỈNH SỬA ĐƯỜNG DẪN SAU __DIR__ CHO ĐÚNG !!!
require_once __DIR__ . '/config/database.php';


// Hàm ghi log trực tiếp (đã comment out như bạn gửi, không cần thay đổi gì ở đây)
/*
if (!function_exists('direct_exec_log_create_email')) { // Đổi tên hàm để tránh xung đột
    function direct_exec_log_create_email($message, $log_path) {
        // ... (nội dung hàm) ...
    }
}
*/
// GHI CHÚ: Để đơn giản, tôi sẽ dùng error_log() cho các log debug trong file này.
