...
$currentLang = strtolower($_REQUEST['lang'] ?? ($_SESSION['lang'] ?? 'vi'));
if ($currentLang !== 'en' && $currentLang !== 'vi') { $currentLang = 'vi'; }

function __t($key, $lang = 'vi') {
    $dict = [
        'vi' => [
            'db_connect_fail'   => 'Lỗi máy chủ: Không thể kết nối cơ sở dữ liệu.',
            'db_save_fail'      => 'Lỗi máy chủ: Không thể lưu yêu cầu gửi email.',
            'unexpected_error'  => 'Đã xảy ra lỗi không mong muốn.',
            'token_revoked'     => 'Token đã hết hạn hoặc bị thu hồi. Vui lòng đăng nhập/ủy quyền lại Gmail.',
            'invalid_credentials'=> 'Thông tin xác thực không hợp lệ. Vui lòng cấu hình lại Gmail OAuth.',
        ],
        'en' => [
            'db_connect_fail'   => 'Server error: Cannot connect to the database.',
            'db_save_fail'      => 'Server error: Unable to save the email request.',
            'unexpected_error'  => 'An unexpected error occurred.',
            'token_revoked'     => 'Token has expired or been revoked. Please sign in / authorize Gmail again.',
            'invalid_credentials'=> 'Invalid credentials. Please reconfigure Gmail OAuth.',
        ]
    ];
...
/**
 * Map raw Google/PHPMailer error messages into localized messages.
 */
function translateGoogleError($rawMessage, $lang = 'vi') {
    $hay = strtolower($rawMessage ?? '');

    if (strpos($hay, 'token has been expired or revoked') !== false) {
        return __t('token_revoked', $lang);
    }
    if (strpos($hay, 'invalid credentials') !== false) {
        return __t('invalid_credentials', $lang);
    }
    // Add more mappings as needed...

    // Fallback generic
    return __t('unexpected_error', $lang);
}
...
if (!$pdo) {
    ...
    exit(json_encode(["success" => false, "message" => __t('db_connect_fail', $currentLang)]));
}
...
try {
    ...
    echo json_encode(["success" => false, "message" => __t('db_save_fail', $currentLang)]);
} catch (Exception $e) {
     error_log("General error in send_email_custom.php for {$log_type}: " . $e->getMessage());
     echo json_encode(['success' => false, 'message' => translateGoogleError($e->getMessage(), $currentLang)]);
     http_response_code(500);
}
